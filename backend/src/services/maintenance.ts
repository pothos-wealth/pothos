...
export function runInboxCleanup(): void {
    try {
        ...
        console.info(`[maintenance] Inbox cleanup complete: ${ptDeleted} parsed_transactions, ${pmDeleted} pending_messages removed`);
    } catch (err) {
        console.error("[maintenance] Inbox cleanup failed:", err instanceof Error ? err.message : String(err));
    }
}
