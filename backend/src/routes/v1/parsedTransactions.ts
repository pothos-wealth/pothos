...
import { parsedTransactions, pendingMessages, transactions, accounts, categories } from "../../db/schema.js";
...
export async function parsedTransactionRoutes(app: FastifyInstance) {
    ...
    app.put("/parsed-transactions/:id", { preHandler: authenticate }, async (request, reply) => {
        ...
        const existing = db
            .select()
            .from(parsedTransactions)
            .where(and(eq(parsedTransactions.id, id), eq(parsedTransactions.userId, request.user.id)))
            .get();
    ...
    app.post("/parsed-transactions/:id/approve", { preHandler: authenticate }, async (request, reply) => {
        const { id } = request.params as { id: string };
        ...
        if (pt.status !== "pending_review") {
            return reply.status(409).send({ error: "Transaction has already been approved or rejected" });
        }

        if (!pt.accountId) {
            return reply.status(400).send({ error: "An account must be selected before approving" });
        }
    ...
    app.post("/parsed-transactions/:id/reject", { preHandler: authenticate }, async (request, reply) => {
        const { id } = request.params as { id: string };
