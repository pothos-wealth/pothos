...
**Single superadmin per instance** - Configured via `SUPERADMIN_EMAIL` env var. On startup, the backend promotes the matching user and demotes everyone else. Idempotent - safe to restart repeatedly. The user must already exist (sign up first, then set the env var and restart).

**Why env var, not DB flag** - For a self-hosted single-instance app, the superadmin is whoever controls the server. Configuring it via `.env` means it cannot be changed through the UI by accident or by a compromised session. The server operator always has final authority.

**Superadmin panel** - Accessible at `/admin`. Guarded by `authenticateAdmin` middleware which chains `authenticate` + `isSuperadmin` check. Redirects non-superadmins to `/dashboard`.

**Admin capabilities** - View system stats, view all users, revoke individual or all sessions for any user, delete any non-superadmin user (cascade-deletes all data). Superadmins cannot delete themselves or each other.

---

## Registration

**Invite code via env var** - `REGISTRATION_CODE` in `.env` controls whether sign-ups require an invite code. If empty or unset, registration is open. If set, the sign-up form shows an invite code field and the backend rejects registrations where the code does not match.

**Why env var** - The invite code is managed by whoever controls the server. No DB storage needed, no UI to accidentally expose it. Changing the code requires a server restart, which is acceptable for a self-hosted tool.

**Admin panel visibility** - The superadmin panel reads `REGISTRATION_CODE` from the environment and displays the current code with a copy button. If unset, it shows an "open registration" notice. This lets the admin easily share the code with new users.

**Frontend behavior** - On load, the sign-up page calls `GET /auth/config` (public, unauthenticated) to check `registrationRequiresCode`. If true, the invite code field appears. If false, no field is shown and no code is sent.
