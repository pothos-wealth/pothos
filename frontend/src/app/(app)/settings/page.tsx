...
export default function SettingsPage() {
	...
	const IMAP_PROVIDERS: Record<string, { host: string; port: number; hint: string }> = {
		gmail: {
			host: "imap.gmail.com",
			port: 993,
			hint: "Requires 2-Step Verification to be enabled. Then generate an App Password at myaccount.google.com/apppasswords",
		},
		outlook: {
		...
		yahoo: {
			...
			port: 993,
			hint: "Use an App Password (Account Security → Generate app password)",
		},
		icloud: {
			host: "imap.mail.me.com",
			port: 993,
			hint: "Use an App-Specific Password (appleid.apple.com → Sign-In and Security)",
		},
	...
	async function handleEmailDisconnect() {
		try {
			...
			setEmailStatus((prev) =>
				prev ? { ...prev, isConfigured: false, isActive: false } : null
			)
			...
			<div className="px-4 py-6 md:px-6 max-w-2xl mx-auto">
				...
				<Card className="mb-4">
					...
					<div className="flex flex-col gap-1.5">
						...
						<p className="text-xs text-fg-muted">
							Currency is set during account creation and cannot be changed
						</p>
					</div>
				</Card>
					...
					{emailSettings ? (
						<div className="flex flex-col gap-3">
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-xs text-fg-muted mt-0.5">
										{emailSettings.host} · {emailSettings.mailbox}
									</p>
									<p className="text-xs text-fg-muted mt-0.5">
							...
							<div className="flex items-center gap-2 pt-1">
								<span className="relative flex h-2 w-2">
									...
									<span
										className={`relative inline-flex rounded-full h-2 w-2 ${emailSettings.isActive ? "bg-income" : "bg-fg-muted"}`}
									/>
								</span>
								<span className="text-xs text-fg-muted">
									{emailSettings.isActive ? "Active" : "Disabled"}
								</span>
								{emailStatus && (
									<span className="ml-auto text-xs text-fg-muted">
										{emailStatus.pendingCount} pending in queue
									</span>
								)}
							</div>
						</div>
						...
						<form onSubmit={handleEmailSave} className="flex flex-col gap-3">
							...
							<div className="flex flex-col gap-1.5">
								...
								/>
								<p className="text-xs text-fg-muted">
									The folder to watch for bank notifications. Usually INBOX.
								</p>
							</div>
						...
						>
							...
							{llmProvider !== "local" && (
								<div className="flex flex-col gap-1.5">
									...
									{llmSettings?.apiKey && !llmApiKey && (
										<p className="text-xs text-fg-muted">
											Leave blank to keep existing key ({llmSettings.apiKey})
										</p>
									)}
								</div>
							)}
							...
							{llmProvider === "local" && (
								<p className="text-xs text-fg-muted bg-bg-3 rounded-xl px-3 py-2">
									Local LLM uses the MCP server with Ollama. Emails will be
									available in the parse queue for local processing.
								</p>
							)}
				...
				<Card>
					...
					>
						...
						<div className="flex flex-col gap-1.5">
							<label htmlFor="new-password" className="text-sm font-medium text-fg">
								New Password
							</label>
							<div className="relative">
								<input
