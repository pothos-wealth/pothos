...
export default function TransactionsPage() {
    ...
    const [txForm, setTxForm] = useState<TxForm>({
        type: 'expense', accountId: '', categoryId: '', amount: '', date: todayISO(), description: '', notes: '',
    })
    const [transferForm, setTransferForm] = useState<TransferForm>({
        fromAccountId: '', toAccountId: '', amount: '', date: todayISO(), description: '',
    })
    ...
    function openAdd() {
        ...
        setTxForm({ type: 'expense', accountId: firstAccount, categoryId: '', amount: '', date: todayISO(), description: '', notes: '' })
        setTransferForm({ fromAccountId: firstAccount, toAccountId: activeAccounts[1]?.id ?? '', amount: '', date: todayISO(), description: '' })
        ...
        <div className="p-6 max-w-5xl mx-auto">
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        !isTransfer && 'cursor-pointer hover:bg-bg-3 transition-colors duration-100'
                                    )}
                                >
                                    <span className={cn(
                                        'w-2 h-2 rounded-full shrink-0',
                                        isTransfer ? 'bg-fg-muted' : isIncome ? 'bg-primary' : 'bg-expense'
                                    )} />
            ...
            {list && list.pagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                    <p className="text-xs text-fg-muted">
                        Page {list.pagination.page} of {list.pagination.totalPages} · {list.pagination.total} transactions
                    </p>
                    ...
                    {(['transaction', 'transfer'] as TxTab[]).map((t) => (
                        <button
                            ...
                            className={cn(
                                ...
                                addTab === t ? 'bg-bg-2 text-fg shadow-sm' : 'text-fg-muted hover:text-fg'
                            )}
                        >
                ...
                <form onSubmit={handleAddSubmit} className="flex flex-col gap-3">
                    ...
                    {addTab === 'transaction' ? (
                        <>
                                ...
                                {(['expense', 'income'] as const).map((t) => (
                                    <button
                                        ...
                                        className={cn(
                                            'flex-1 py-1.5 rounded-lg text-sm font-medium transition-colors duration-150',
                                            txForm.type === t
                                                ? t === 'expense' ? 'bg-expense-light text-expense' : 'bg-accent-light text-primary'
                            ...
                            <select value={txForm.accountId} onChange={(e) => setTxForm((f) => ({ ...f, accountId: e.target.value }))} required className={inputCls}>
                                <option value="">Select account</option>
                                {activeAccounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
                            </select>
                            <select value={txForm.categoryId} onChange={(e) => setTxForm((f) => ({ ...f, categoryId: e.target.value }))} className={inputCls}>
                                <option value="">No category</option>
                                {categories.filter((c) => c.type === txForm.type || c.type === 'neutral').map((c) => (
                        ...
                        <>
                            <select value={transferForm.fromAccountId} onChange={(e) => setTransferForm((f) => ({ ...f, fromAccountId: e.target.value }))} required className={inputCls}>
                                <option value="">From account</option>
                                {activeAccounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
                            </select>
                            <select value={transferForm.toAccountId} onChange={(e) => setTransferForm((f) => ({ ...f, toAccountId: e.target.value }))} required className={inputCls}>
