...
import { Check, X, Pencil, ChevronLeft, ChevronRight, Inbox, TrendingDown, TrendingUp } from 'lucide-react'
...
import type { Account, Category, ParsedTransaction, ParsedTransactionList, PendingMessage } from '@/lib/types'
...
export default function InboxPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (activeTab === 'unprocessed') {
                ...
                const result = await api.parsedTransactions.list({ status: activeTab, page, limit: 20 })
    ...
    const isEmpty = activeTab === 'unprocessed' ? rawMessages.length === 0 : parsedData?.data.length === 0
    ...
    return (
        ...
        <div className="px-4 py-6 md:px-6 max-w-3xl mx-auto">
            <div className="mb-6">
                ...
                <p className="text-sm text-fg-muted mt-0.5">Review and parse your bank emails</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    {(activeTab === 'pending_review' || activeTab === 'unprocessed') && (
                        <p className="text-xs text-fg-muted mt-1">Connect an email account in Settings to start importing</p>
                    )}
                </div>
                ...
                <div className="flex flex-col gap-3">
                    {rawMessages.map((msg) => (
                        <div
                            ...
                            onClick={() => { setParseItem(msg); setParseForm(defaultParseForm(msg)); setParseError('') }}
                        >
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-xs text-fg-muted mt-0.5">{formatDate(msg.createdAt)}</p>
                                    <p className="text-xs text-fg-muted mt-1.5 line-clamp-2 leading-relaxed">
                                        {msg.rawContent.slice(0, 160)}
                                    </p>
                                </div>
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleDismiss(msg) }}
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <span className={`text-base font-semibold ${pt.type === 'income' ? 'text-income' : 'text-expense'}`}>
                                            {pt.type === 'income' ? '+' : '-'}{fmt(pt.amount)}
                                        </span>
                                        <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded-full ${
                                            pt.type === 'income' ? 'bg-income-light text-income' : 'bg-expense-light text-expense'
                                        }`}>
                                            {pt.type}
                                        </span>
                                    </div>
                                    <p className="text-sm font-medium text-fg mt-0.5 truncate">{pt.description}</p>
                                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                                        <span className="text-xs text-fg-muted">{formatDate(pt.date)}</span>
                                        {pt.accountName ? (
                                            <span className="text-xs text-fg-muted">· {pt.accountName}</span>
                                        ) : (
                                            <span className="text-xs text-expense">· No account selected</span>
                                        )}
                                        {pt.categoryName && (
                                            <span className="text-xs text-fg-muted">· {pt.categoryName}</span>
                                        )}
                                    </div>
                                    {pt.emailSubject && (
                                        <p className="text-xs text-fg-muted mt-1 italic truncate">"{pt.emailSubject}"</p>
                                    )}
                                </div>
                                ...
                                {activeTab === 'pending_review' && (
                                    <div className="flex items-center gap-1.5 shrink-0">
                                        <button
                                            onClick={() => { setEditItem(pt); setEditForm(defaultEditForm(pt)); setEditError('') }}
                                        ...
                                        <button
                                            ...
                                            title={pt.accountId ? 'Approve' : 'Select an account first'}
                                        >
                                            <Check size={15} />
                                        </button>
                                        <button
                    ...
                    {parsedData && parsedData.pagination.totalPages > 1 && (
                        <div className="flex items-center justify-between pt-2">
                            ...
                            <span className="text-xs text-fg-muted">
                                Page {parsedData.pagination.page} of {parsedData.pagination.totalPages}
                            </span>
                            <button
                                onClick={() => setPage((p) => Math.min(parsedData.pagination.totalPages, p + 1))}
            ...
            {parseItem && parseForm && (
                <Modal open={!!parseItem} title="Parse Email" onClose={() => setParseItem(null)}>
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="rounded-xl bg-bg-3 border border-border overflow-hidden">
                            <div className="px-4 py-3 border-b border-border">
                                ...
                                <p className="text-xs text-fg-muted mt-0.5">{formatDate(parseItem.createdAt)}</p>
                            </div>
                        ...
                        <form onSubmit={handleParseSubmit} className="flex flex-col gap-3">
                            ...
                            <div className="flex rounded-xl overflow-hidden border border-border">
                                <button
                                    type="button"
                                    onClick={() => setParseForm({ ...parseForm, type: 'expense' })}
                                ...
                                <button
                                    type="button"
                                    onClick={() => setParseForm({ ...parseForm, type: 'income' })}
                                    className={`flex-1 py-2.5 text-sm font-semibold flex items-center justify-center gap-2 transition-colors duration-150 ${
                            ...
                            <div className="flex gap-2">
                                <div className="flex flex-col gap-1.5 flex-1">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setParseForm({ ...parseForm, amount: e.target.value })}
                                        placeholder="0.00"
                                ...
                                <div className="flex flex-col gap-1.5 w-36">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setParseForm({ ...parseForm, date: e.target.value })}
                                        className={`${inputCls} w-full`}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setParseForm({ ...parseForm, description: e.target.value })}
                            ...
                            <div className="flex gap-2">
                                <div className="flex flex-col gap-1.5 flex-1">
                                    <label className="text-sm font-medium text-fg">Account <span className="text-expense">*</span></label>
                                    <select
                                        required
                                        value={parseForm.accountId}
                                        onChange={(e) => setParseForm({ ...parseForm, accountId: e.target.value })}
                                        className={`${inputCls} w-full`}
                                    >
                                ...
                                <div className="flex flex-col gap-1.5 flex-1">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setParseForm({ ...parseForm, categoryId: e.target.value })}
                                        className={`${inputCls} w-full`}
                                    >
                                        <option value="">None</option>
                                        {categories
                                            .filter((c) => c.type === parseForm.type || c.type === 'neutral')
                                            .map((c) => (
                            ...
                            {/* Notes */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-sm font-medium text-fg">Notes <span className="text-fg-muted font-normal">(optional)</span></label>
                                <input
                                    type="text"
                                    value={parseForm.notes}
                                    onChange={(e) => setParseForm({ ...parseForm, notes: e.target.value })}
                            ...
                            <div className="flex items-center gap-3 mt-1">
                                <button
                                    type="button"
                                    onClick={() => { setParseItem(null); handleDismiss(parseItem) }}
            ...
            {/* ── Edit Modal ────────────────────────────────────────────────────── */}
            {editItem && editForm && (
                <Modal open={!!editItem} title="Edit Transaction" onClose={() => setEditItem(null)}>
                    <form onSubmit={handleEditSubmit} className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            >
                                ...
                                {categories
                                    .filter((c) => c.type === editForm.type || c.type === 'neutral')
                                    .map((c) => (
                                        <option key={c.id} value={c.id}>{c.icon ? `${c.icon} ` : ''}{c.name}</option>
                                    ))}
                            </select>
