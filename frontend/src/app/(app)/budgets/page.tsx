...
export default function BudgetsPage() {
	...
	function openEdit(budget: BudgetWithSpent) {
		...
		setForm({
			...
			isCommitted: budget.isCommitted,
		})
		setError("")
		setModalOpen(true)
	}
	...
	const totalCommitted = budgets
		.filter((b) => b.isCommitted)
			...
			<div className="px-4 py-6 md:px-6 max-w-4xl mx-auto">
				...
				{accounts.length > 0 && (
					<div className="grid grid-cols-3 gap-3 mb-6">
						...
						<div
							className={`border rounded-xl px-4 py-3 ${freeToSpend < 0 ? "bg-expense-light border-expense" : "bg-income-light border-income"}`}
						>
							<p className="text-xs text-fg-muted mb-1">Free to Spend</p>
							<p
								className={`text-sm font-semibold ${freeToSpend < 0 ? "text-expense" : "text-income"}`}
							>
				...
				>
					<form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
						...
						<div className="flex flex-col gap-1.5">
							<label htmlFor="budget-amount" className="text-sm font-medium text-fg">
								Budget Amount
							</label>
							<input
								id="budget-amount"
						...
						<label className="flex items-center gap-3 cursor-pointer select-none">
							<input
								...
								onChange={(e) =>
									setForm((f) => ({ ...f, isCommitted: e.target.checked }))
								}
								className="w-4 h-4 rounded accent-primary"
							/>
