	...
	return (
		<PageTransition>
					...
					<Card>
						...
						{settings?.registrationCode ? (
							<div className="flex items-center justify-between gap-2">
								<p className="text-lg sm:text-2xl font-bold text-fg font-mono truncate">
									{settings.registrationCode}
								</p>
								<button
									onClick={() => copyCode(settings.registrationCode!)}
							...
							<p className="text-lg sm:text-2xl font-bold text-fg-muted">Open</p>
						)}
					</Card>
				</div>
						...
						<table className="w-full text-sm">
							<thead>
								<tr className="border-b border-border">
									<th className="text-left px-5 py-3 text-xs font-semibold text-fg-muted">
										Email
									</th>
									<th className="text-left px-5 py-3 text-xs font-semibold text-fg-muted">
										Joined
									...
									<th className="text-right px-5 py-3 text-xs font-semibold text-fg-muted">
										Accounts
									</th>
									<th className="text-right px-5 py-3 text-xs font-semibold text-fg-muted">
										Transactions
							...
							<tbody className="divide-y divide-border">
								{users.map((u) => (
									<tr
										key={u.id}
										className="hover:bg-bg-3 transition-colors duration-100"
									>
										...
										<td className="px-5 py-3 text-fg text-right">
											{u.accountCount}
										</td>
										<td className="px-5 py-3 text-fg text-right">
											{u.transactionCount}
			...
			<Modal
				...
				className="max-w-lg"
			>
				{sessionError && <p className="text-sm text-expense mb-4">{sessionError}</p>}
				{sessionsLoading ? (
