...
export default function AccountsPage() {
    ...
    function openEdit(account: Account) {
        setEditing(account)
        setForm({ name: account.name, type: account.type, initialBalance: String(account.initialBalance / 100) })
        setError('')
        setModalOpen(true)
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            if (editing) {
                const updated = await api.accounts.update(editing.id, { name: form.name, type: form.type })
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-sm text-fg-muted mt-0.5">
                        Total balance:{' '}
                        <span className="font-semibold text-fg">{formatCurrency(totalBalance)}</span>
                    </p>
                </div>
                <button
                    onClick={openAdd}
                        ...
                        <Card key={account.id} className={account.isActive ? '' : 'opacity-60'}>
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-fg-muted mt-0.5">{account.type}</p>
                                </div>
                                <p className="text-xl font-bold text-fg">{formatCurrency(account.balance)}</p>
                            </div>
                            ...
                            <div className="flex items-center gap-1 mt-4 pt-3 border-t border-border">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 text-xs font-medium text-fg-muted hover:text-fg px-2.5 py-1.5 rounded-lg hover:bg-bg-3 transition-colors duration-150"
                                >
                                    {account.isActive ? <Lock size={13} /> : <LockOpen size={13} />}
